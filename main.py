...
if f.read() == "":
    ...
    with open(fileName + ".hof","a") as write:
        write.write("#####\n\n\n")
        write.write("[name]\n")
        hofName = input("Enter the depot name: ")
        write.write(hofName)
        write.write("\n\n[servicetrip]\n")
        write.write("Blank\n\n")
        write.write("stringcount_terminus\n")
        write.write("6\n\n")
        write.write("stringcount_busstop\n")
        write.write("4\n\n")
        write.write("####################\n")
        addTermExit = input("Do you want to automatically addTerminusAllExit now? [Y/n]")
        if addTermExit == "Y":
            write.write("[addterminus_allexit]\n51000\nNIS\n\n\n\n\nM1.bmp\nM1\n\n")
        else:
            print("")
        termNo = input("Please calculate and enter the total terminus number: ")
        try:
            int(termNo)
        except:
            print("Invalid input.")
            quit()
        i = 0
        for i in range(int(termNo)):
            write.write(term + "\n")
            IBIS = input("Enter the IBIS code: ")
            write.write(IBIS + "\n")
            termName = input("Enter the terminus name: ")
            write.write(termName + "\n\n\n\n\n")
            bmpName = input("Enter the relevant bmp file name: ")
            write.write(bmpName + "\n")
            lastPos = bmpName.find(".")
            name = bmpName[0:lastPos]
            write.write(name + "\n\n")
            i = i + 1
        addbs = True
        while addbs == True:
            ...
            if isDDU == "Y":
                ...
                if isPrice == "Y":
                    ...
                    if len(chinStopName) > 6:
                        ...
                        downloadWelcomeImage(gen8, "8words\\" + bsName + ".bmp")
                        if bsName.find("_") != -1:
                            bsName = bsName.replace("_","")
                        downloadWelcomeImage(gen, "6words\\" + bsName + ".bmp")
                        downloadWelcomeImage(gen8, "8words\\" + bsName + ".bmp")
